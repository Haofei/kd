...
func checkSpeaker() string {
	...
	case "darwin":
		checkList = []string{"afplay"}
	case "windows":
		checkList = []string{"ffplay"}
	}
	...
	if speakerProgram == "" && runtime.GOOS == "windows" {
		...
		d.EchoWarn("推荐安装ffmpeg支持无弹窗播放声音")
	}
	if speakerProgram != "" {
...
func playAudio(filePath string) error {
	...
	if runtime.GOOS == "windows" {
		...
		case "ffplay":
			// cmd = exec.Command("powershell", "-c", fmt.Sprintf("Start-Process ffplay -nodisp -autoexit -loglevel quiet '%s'", filePath))
	...
	if output, err := cmd.CombinedOutput(); err != nil {
		// zap.S().Errorw("Error playing audio", "file", filePath, "error", err, "output", string(output))
